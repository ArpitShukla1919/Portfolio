...
import work from '/img/projects.png'
import Gfonts from '../assets/img/gfonts.png'
import Icon2 from '../assets/img/icon2.png'

import { BiPowerOff } from "react-icons/bi";
import { FaCamera, FaGit, FaGithub, FaJs, FaLinkedin, FaNode, FaReact, FaShopify } from "react-icons/fa";
import { PiPencilCircle } from "react-icons/pi";
import { GiCircleSparks } from "react-icons/gi";
import { MdOutlineMobileFriendly } from "react-icons/md";
import { TbBrandMongodb, TbBrandMysql, TbBrandNextjs, TbWorld } from "react-icons/tb";
...
import { SiCodechef, SiExpress, SiMongodb } from 'react-icons/si'
...
function Home() {
  ...
  return (
    <div className='bg-[#0F0F0F] pt-14 space-y-5'>
      ...
      <div className='grid px-4 space-y-4 lg:space-x-5 md:container-md lg:grid-cols-2 md:grid-cols-1 md:space-y-3 lg:px-10'>
        ...
        <motion.div
          ...
          <div className='flex flex-col items-start space-y-3'>
            <p className='sm:self-start md:self-start lg:self-start text-slate-500'>A Web Programmer</p>
            <h2 className='self-start text-5xl text-white'>Arpit Shukla</h2>
            <p className='mt-3 sm:self-start md:self-start lg:self-start text-slate-500'>I am a Web Programmer based in Lucknow</p>
          </div>
      ...
      <div className='container grid w-auto h-full grid-cols-1 gap-4 px-6 mx-auto lg:grid-cols-3 lg:px-10 md:container-md md:grid-cols-2 md:grid-rows-2 lg:grid-rows-1 '>
        ...
        >
          {/* Icon Grid */}
          <div className="grid grid-cols-3 gap-6 items-center justify-items-center mb-6 text-4xl text-white md:gap-4 lg:gap-6">
            <FaReact />
            <FaJs />
            <TbBrandMongodb />
            <TbBrandNextjs />
          ...
          <div className="flex flex-row items-center justify-between">
            ...
            {/* Icon */}
            <div className="flex items-center text-3xl text-gray-700 group-hover:text-white transition ease-in-out duration-300">
              <BiPowerOff className="rotate-90" />
            </div>
          </div>
        </motion.div>
      ...
      <div className='container grid w-auto h-full grid-cols-1 gap-4 px-6 mx-auto lg:px-10 md:container-md lg:grid-cols-2 md:grid-cols-1 '>
        ...
        <motion.div
          ...
          transition={{ duration: 1 }}
          viewport={{ once: true, amount: 0.1 }}
          className='grid content-center grid-cols-1 gap-3 p-7 bg-gradient-to-r from-slate-700 to-slate-900 rounded-3xl lg:grid-cols-2 md:grid-cols-2'>
          <ScrollTrigger onEnter={() => setCountOn(true)} onExit={() => setCountOn(false)}>
            <div className='flex flex-col items-center justify-center p-3 shadow-sm bg-gradient-to-r from-slate-700 to-slate-900 rounded-3xl shadow-black '>
              {CountOn && <h1 className='pb-6 text-5xl text-white '><CountUp start={0} end={1} duration={5} /></h1>}
          ...
          <ScrollTrigger>
            <div className='flex flex-col items-center justify-center p-3 shadow-sm bg-gradient-to-r from-slate-700 to-slate-900 rounded-3xl shadow-black'>
              {CountOn && <h1 className='pb-6 text-5xl text-white '>+<CountUp start={0} end={9} duration={5} /></h1>}
